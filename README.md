...
This repository provides an out-of-the-box ZMK configuration tailored exclusively for the **"MiaoMiao" custom wireless Charybdis (RGB Version)** split trackball keyboard. It natively supports real-time keymap editing via ZMK Studio, optimizes wireless connectivity, and maintains full backward compatibility with the official soldered version.

> **⚠️ Hardware Notice**: This configuration is strictly dedicated to the **RGB version**. Please verify your hardware before flashing. Be aware that the RGB matrix inherently results in higher power consumption compared to the standard non-RGB version.
